...
xkb_resolve () {
    ...
    xkbcomp $1 $DISPLAY 2> _xkbcomp_error.log
...
xkb_test_compilation () {
    xkb_resolve $1 > _xkb_test_compilation_out.txt

    ret=$?
    if [ $ret -ne 0 ]
    then
        cat _xkb_test_compilation_out.txt
    fi

    rm _xkb_test_compilation_out.txt
}
...
xkb_diff () {
    setxkbmap -print > _xkb_diff_curr_keymap_bak.xkb
    ...
    then
        ...
        # Restore backup keymap
        xkbcomp _xkb_diff_curr_keymap_bak.xkb $DISPLAY 2> /dev/null
        return 1
    fi
    xkbcomp -xkb $DISPLAY _xkb_diff_resolved_keymap_1.xkb

    xkbcomp $2 $DISPLAY 2> _xkbcomp_error.log
    if [ -s _xkbcomp_error.log ]
    then
        cat _xkbcomp_error.log
        echo There was an issue compiling "$2".
        rm _xkbcomp_error.log
    ...
    xkbcomp -xkb $DISPLAY _xkb_diff_resolved_keymap_2.xkb

    diff _xkb_diff_resolved_keymap_1.xkb _xkb_diff_resolved_keymap_2.xkb
    files_difer=$?
    ...
    then
        rm _xkbcomp_error.log
        rm _xkb_diff_curr_keymap_bak.xkb
        rm _xkb_diff_resolved_keymap_1.xkb _xkb_diff_resolved_keymap_2.xkb
    fi
}
